...
func getLocation(rw http.ResponseWriter, req *http.Request, p httprouter.Params) {
    ...
    if err := newSession().DB("tripplan").C("location").FindId(dataid).One(&responseObj); err != nil {
        rw.WriteHeader(404)
        return
    }
...
func getCordinates(x string) (y coordinate) {
    ...
    if err := newSession().DB("tripplan").C("location").Find(bson.M{"_id": bson.ObjectIdHex(x)}).One(&obj1); err != nil {
        z := coordinate{}
    return z
}
    p := coordinate{
...
func shortPath (rw http.ResponseWriter, req *http.Request, p httprouter.Params){
   ...
   Tripdata := map[string]Data{}
      for i:=0;i<len(x);i++{
      url := fmt.Sprintf("https://sandbox-api.uber.com/v1/estimates/price?start_latitude=%f&start_longitude=%f&end_latitude=%f&end_longitude=%f&server_token=6aSSJPLoTBCmMImqNzyl_6DmPIz6KziFSQYgbBkU",start_lat,start_lng,x[i].lat,x[i].lng)
  ...
  T.Best_route_location_ids = append(T.Best_route_location_ids,da.id)
   ...
   z = getCordinates(Start)
    ...
    x.lng=y.lng
       Tripdata := map[string]Data{}
      url := fmt.Sprintf("https://sandbox-api.uber.com/v1/estimates/price?start_latitude=%f&start_longitude=%f&end_latitude=%f&end_longitude=%f&server_token=6aSSJPLoTBCmMImqNzyl_6DmPIz6KziFSQYgbBkU",start_lat,start_lng,x.lat,x.lng)
      d:=pricetoBegin(url)
      Tripdata[Start] = d
...
newSession().DB("tripplan").C("tripData").Insert(T)
...
func trip(rw http.ResponseWriter, req *http.Request, p httprouter.Params) {
    ...
    if err := newSession().DB("tripplan").C("tripData").FindId(dataid).One(&responseObj); err != nil {
        rw.WriteHeader(404)
        return
    }
...
client := uber.NewClient("6aSSJPLoTBCmMImqNzyl_6DmPIz6KziFSQYgbBkU")
tripid :=  p.ByName("tripid")
    if !bson.IsObjectIdHex(tripid) {
        rw.WriteHeader(404)
        return
...
nextid = Locids[0]
  ...
  newSession().DB("tripplan").C("tripData").Update(bson.M{"_id":dataid }, bson.M{"$set": bson.M{ "status": "Requesting"}})
  startid = nextid
...
eta:=etaData(start_lat,start_lng,productid)
  ...
  res, _ := json.Marshal(reply)
    ...
    Uber_wait_time_eta: 0 ,
     ...
     newSession().DB("tripplan").C("tripData").Update(bson.M{"_id":dataid }, bson.M{"$set": bson.M{ "status": "Finished"}})
     nextid=""
  res, _ := json.Marshal(reply)
    rw.Header().Set("Content-Type", "application/json")
...
func newSession() *mgo.Session {
    ...
    s, err := mgo.Dial("mongodb://aditya42:aditya42$@ds043694.mongolab.com:43694/tripplan")
